...
def nickname_change_2(conn):
    ...
    if flask.request.method == 'POST':
        ...
        return easy_minify(flask.render_template(skin_check(),
            imp = [load_lang('nickname'), wiki_set(), custom(), other2([0, 0])],
            data =  '''
                    <form method="post">
                        <input placeholder="''' + load_lang('nickname') + '''" name="id" type="text">
                        <hr class=\"main_hr\">
                        ''' + captcha_get() + '''
                        <button type="submit">''' + load_lang('닉네임 변경') + '''</button>
                        <hr class=\"main_hr\">
                        ''' + http_warring() + '''
                    </form>
                    ''',
